...
class OlistSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        # No link to the details page is given, Need to splice links
    ...
    def product_detail(self, response):
        ...
        jsn = json.loads(jsn)["detail"]["detailData"].get("info")

        for i in jsn["attrs"]:  # Loop out the required data
            key = i.get("label").lower()
            value = i.get("value")
            if key == "make":
                output["make"] = value
            elif key == "model":
        ...
        output["vehicle_disclosure"] = jsn["content"]
        ...
        output["state_or_province"] = jsn["state_name"]
