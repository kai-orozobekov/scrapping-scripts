...
class BeforwardsingaporeSpider(scrapy.Spider):
    name = "beforwardsingapore"
    download_timeout = 120

    def start_requests(self):
        urls = [
            "https://www.beforward.jp/stocklist/page=1/sortkey=n/stock_country=35",
        ]
    ...
    def detail(self, response):
        ...
        if (
            ...
            if response.xpath('//*[@id="spec"]/table/tr[5]/td[1]').get().strip() != "-":
                ...
                output["model"] = response.xpath(
                    '//*[@id="bread"]/li[4]/a/text()'
                ).get()

                # description head
                des_key = response.xpath(
                    '//td[@class="specs-pickup-text"]/text()'
                ).getall()
