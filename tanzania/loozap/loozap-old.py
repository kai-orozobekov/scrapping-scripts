...
class LoozapSpider(scrapy.Spider):
    name = 'loozap'
    start_urls = ['https://tz.loozap.com/category/autos-cars-vehicles-and-trucks/used-cars-new-cars?page=1']
    ...
    async def parse(self, response: Response):
        ...
        current_page = int(response.url.split("page=").pop())
        if next_button and current_page < 3:
            yield scrapy.Request(
                url=next_button,
                callback=self.parse,
                meta={
            ...
            if "Location" in key:
                ...
                if price:
                    ...
                    if true_price.isdigit():
                        ...
                        currency = price.strip().split(" ")[1]
                        if currency.upper() == "TSH":
                            output["currency"] = "TZS"
