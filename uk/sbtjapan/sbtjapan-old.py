...
class SbtjapanSpider(scrapy.Spider):
    ...
    start_urls = [
        "https://www.sbtjapan.com/used-cars/?custom_search=uk_inventory&location=uk&p_num=1#listbox"
    ]
    ...
    def parse(self, response):
        ...
        if carlist:
            ...
            carlist_pager = tree.xpath(
                "//div[@class='carlist_pager']//li/a/text()"
            ...
            pagination_links = [
                response.url.replace(f"p_num={current_page}", f"p_num={i}")
        ...
        for i in range(len(form_data_th)):  # parse form data
            ...
            if "chassis" in key.lower():
                ...
                if location and "-" in location:
                    ...
                    if location.split("-")[1].strip() == "UK":
                        output["country"] = "GB"
                elif "-" not in location:
                    if location.strip() == "UK":
                        output["country"] = "GB"
